    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, data_type: str,
        ...
        for dia_idx in dial_data:
            for dt_i, dia_turn in enumerate(dia_idx["dialogue"]):
                ...
                slot_value = [d for d in dia_turn["turn_label"]]
                # print('slot_value for turn {}'.format(slot_value))

                if len(slot_value) != 0:
                    for s_v_list in slot_value:
                        # print('s_v_list {}'.format(s_v_list))
                        s_v = '-'.join(s_v_list)
                        # print('d-s-v {}'.format(s_v))
                        turn_slot_val = current_turn + ' ' + s_v
                        # print('turn_slot_val {}'.format(turn_slot_val))
                        input_ids = tokenizer(turn_slot_val, add_special_tokens=True, truncation=True,
                                              max_length=block_size)
                        example = {"input_ids": torch.tensor(input_ids["input_ids"]), "dst_labels": torch.tensor(1)}
                        self.examples.append(example)

                        # generate negative example
                        domain_slot = s_v_list[0]
                        # print('in negative example {}'.format(domain_slot))
                        if 'pricerange' in domain_slot:
                            domain_slot = 'hotel-price range'
                        elif 'arriveby' in domain_slot:
                            domain_slot = 'train-arrive by'
                        elif 'leaveat' in domain_slot:
                        ...
                        if value not in s_v_list[1]:
                            ...
                            input_ids = tokenizer(turn_slot_val, add_special_tokens=True, truncation=True,
                                                  max_length=block_size)
                            example = {"input_ids": torch.tensor(input_ids["input_ids"]), "dst_labels": torch.tensor(0)}
                            self.examples.append(example)
...
class TrainerDST(Trainer):
    ...
    def prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description, disable=disable_tqdm):
            ...
            if logits is not None:
                all_logits.extend(torch.sigmoid(logits).cpu().detach().numpy().tolist())
            if labels is not None:
        ...
        uni, counts = np.unique(label_ids, return_counts=True)
        # print('Unique values {} of count {}'.format(uni, counts))

        conf = metrics.confusion_matrix(label_ids, preds)
        ...
        metrics1["f1_score"] = metrics.f1_score(label_ids, preds)
        # print('f1_score {}'.format(metrics1["f1_score_micro"]))
        ...
        metrics1["confusion_matrix"] = conf
...
class GPT2LMandDST(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.binary_classifier = nn.Linear(config.n_embd, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        dst_logits = self.binary_classifier(hidden_states[:, -1, :])
        ...
        # print('dst_logits {}'.format(dst_logits.view(1, -1).squeeze()))
        # print('dst_labels {}'.format(dst_labels))
        loss_fct = BCEWithLogitsLoss()
        dst_loss = loss_fct(dst_logits.view(1, -1).squeeze(), dst_labels.view(-1, dst_labels.size(-1)).squeeze())
...
def get_dataset(
    ...
    if args.line_by_line:
        return DialogTurnDataset(tokenizer=tokenizer, file_path=file_path, block_size=args.block_size,
                                 data_type=data_type, onto_file_path=args.onto_file_path)
    else:
        return TextDataset(
            tokenizer=tokenizer,
            file_path=file_path,
...
def main():
    ...
    print('train_dataset {}'.format(train_dataset[:10]))
    ...
    print('eval_dataset {}'.format(eval_dataset[:10]))
        ...
        if trainer.is_world_master():
            with open(output_eval_file, "w") as writer:
                ...
                for key in sorted(result.keys()):
                    ...
                    writer.write("%s = %s\n" % (key, str(result[key])))
                for key2 in eval_output.keys():
                    writer.write("***** All DST binary eval results *****")
                    logger.info("  %s = %s", key2, str(eval_output[key2]))
                    writer.write("%s = %s\n" % (key2, str(eval_output[key2])))
