...
class Dstc7Trainer(Trainer):
    ...
    def generate_i(self, eval_dataset: Optional[Dataset] = None, tokenizer=None):
        model = self.model
        ...
        for inputs in tqdm(eval_dataloader, desc="generation", disable=False):
            ...
            responses = model.generate(input_ids=inputs["input_ids"], max_length=85, do_sample=True, top_k=15,
                                                       top_p=0.95, num_return_sequences=1, early_stopping=True)
            print('responses are {}'.format(responses))
            dec_res_list = []
            for idx, res in enumerate(responses):
                dec_resp = tokenizer.decode(res, skip_special_tokens=True)
                dec_res_list.append(dec_resp)

            print('response list {}'.format(dec_res_list))
            for idx, inp in enumerate(inputs["input_ids"]):
                print('idx, inp {}, {}'.format(idx, inp))
                key_gen[inputs["key"][idx]] = dec_res_list[idx]

            # print('generated resp {}'.format(responses))
            # print('Key and resp map {}'.format(key_gen))

        with open('/work-ceph/sbariker/data/eval_dsct7/response_hyp.txt', 'w') as file:
            file.write(json.dumps(key_gen))
...
def main():
    ...
    trainer.generate_i(eval_dataset=eval_dataset, tokenizer=tokenizer)
