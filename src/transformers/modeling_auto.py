...
MODEL_WITH_LM_AND_SOFT_DEBIAS_MAPPING = OrderedDict(
    [
        (GPT2Config, GPT2DoubleHeadsModelSoftDebiasing),
    ]
)

MODEL_WITH_LM_AND_HARD_DEBIAS_MAPPING = OrderedDict(
    [
        (GPT2Config, GPT2DoubleHeadsModelHardDebiasing),
    ]
)

MODEL_WITH_LM_AND_EQ_LOSS_RELG2_MAPPING = OrderedDict(
    [
        (GPT2Config, GPT2DoubleHeadsModelReligion2EqLoss),
    ]
)

MODEL_WITH_LM_AND_CLASSIFIER_MAPPING = OrderedDict(
    [
        (GPT2Config, GPT2DoubleHeadsModelCustomClassifier),
    ]
)
...
class AutoModelWithLMAndDebiasHead:
    r"""
    ...
    def from_config(cls, config):
        ...
        mappings = [MODEL_WITH_LM_AND_EQ_LOSS_MAPPING, MODEL_WITH_LM_AND_EQ_LOSS_RELG2_MAPPING, MODEL_WITH_LM_AND_HARD_DEBIAS_MAPPING,
                    MODEL_WITH_LM_AND_SOFT_DEBIAS_MAPPING, MODEL_WITH_LM_AND_CLASSIFIER_MAPPING]

        for mapping in mappings:
            # print(mapping)
            # print(mapping.values())
            if debiasing_head in mapping.values():
                MODEL_WITH_LM_AND_DEBIAS_HEAD_MAPPING = mapping

        for config_class, model_class in MODEL_WITH_LM_AND_DEBIAS_HEAD_MAPPING.items():
            if isinstance(config, config_class):
                return model_class(config)
        raise ValueError(
            "Unrecognized configuration class {} for this kind of AutoModel: {}.\n"
            "Model type should be one of {}.".format(
                config.__class__, cls.__name__, ", ".join(c.__name__ for c in MODEL_WITH_LM_AND_DEBIAS_HEAD_MAPPING.keys())
            )
        )
